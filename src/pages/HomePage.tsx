...
import { TrendingUp, Target, Calendar, Activity, Users, Trophy, BarChart3, Timer, Flame, Heart, Zap } from "lucide-react";
const HomePage = () => {
  const {
    profile
  } = useUserProfile();
  ...
  useEffect(() => {
    ...
    if (savedStats) {
      ...
      // Initialize stats only if profile is available
  ...
  return <div className="pb-16 pt-14 bg-background min-h-screen">
      <AppHeader />
      
      <div className="px-3 py-4 space-y-4 bg-slate-950">
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-lg text-corpoideal-purple text-slate-50">Ações Rápidas</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              {quickActions.map((action, index) => <Button key={index} onClick={action.action} className={`${action.color} hover:opacity-90 h-20 flex flex-col items-center justify-center text-white border-0`}>
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-lg text-corpoideal-purple flex items-center text-slate-50">
          ...
          <CardContent className="space-y-4">
            <div>
              <div className="flex justify-between items-center mb-2">
                <span className="text-sm font-medium text-slate-50">Treinos Concluídos</span>
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                <span className="text-sm font-medium text-slate-50">Meta de Calorias</span>
